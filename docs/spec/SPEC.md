...
- Date: 2026-07-23 (v0.8 scoped FR-9's undecodable-network arc by mode and made NFR-1 consistent with it, per ADR-0019, from the issue #26 hygiene pass; v0.7 dated 2026-07-13 closed OQ-5 into NFR-2's concrete budget from the M1 measurements; v0.6 added FR-22's exit-code contract; v0.5 recorded FR-14's mode split for the Landlock leg per ADR-0015; v0.4 deferred the ARM64 target per ADR-0014; v0.3 revised FR-14's kernel floor to 5.19 per ADR-0012; v0.2 dated 2026-07-07)
...
- **FR-9** — Every **decision** MUST be **fail-closed**: any supervisor error, crash, or decision timeout resolves the pending action to **deny** (NFR-1).
  ...
  Every other arc remains fail-closed in both modes without exception: a filesystem or process-creation fact the supervisor cannot decode, a recorder-write failure, a supervisor crash, a decision timeout, and every syscall establishing an un-mediated I/O path (SR-4).
